...
from social_choice.rules import equal_shares
from social_choice.election_model import *
from social_choice.build_elections import *
...
class MesWrapper(PrunableMixtralSparseMoeBlockWrapper):
    def start_caching(self):
        self.cache_logits = True

    def enumerate(self):
        self.cache_space.prepare_for_loader()
        utils = self.cache_space.alphas
        elections = build_elections(utils, self.r)
        winners = equal_shares(elections)
        winner_indices = torch.tensor([int(c.id) for c in winners], dtype=torch.long)
        self.experts_to_drop = [i for i in range(self.model.num_experts) if i not in winner_indices]
        del self.cache_space
        return {}
