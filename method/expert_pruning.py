...
def get_layerwise_pruning(wrapper):
    wrapper = getattr(mdl, wrapper)
    def layerwise_pruning(model: MixtralForCausalLM, calib_loader: DataLoader, args: Namespace):
        ...
        for ix in range(0, len(layer_list), LAYER_BATCH):
            layers_batch = layer_list[ix: ix + LAYER_BATCH]

            for l, layer in enumerate(layers_batch, start=ix):
                layer.block_sparse_moe = wrapper(
                    layer.block_sparse_moe, r=args.r)
                layer.block_sparse_moe.start_caching()
